...
2. Counts: `PASS=<n> FAIL=<n> SKIP=<n> WARN=<n> TIMEOUT=<n>`.
3. The names of any FAIL/TIMEOUT methods (one per line), so the orchestrator can decide whether to fix the spec before Phase 9.
